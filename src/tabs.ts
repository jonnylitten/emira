...
import type { DetectedElement, LabelMap } from "./types.js";
...
export class TabRegistry {
  ...
  getActive(): TabState {
    if (this.activeId === null) {
      throw new Error(
        "No active tab. The browser context has no open pages — call open_tab to start one.",
      );
    }
  ...
  getById(id: number): TabState {
    const state = this.tabs.get(id);
    if (!state) {
      const open = Array.from(this.tabs.keys()).sort((a, b) => a - b);
      throw new Error(
        `Tab ${id} not found. Open tabs: ${open.length ? open.join(", ") : "(none)"}. Call list_tabs to inspect.`,
      );
    }
    return state;
  }
   ...
   */
  async open(url?: string, wait_ms?: number): Promise<TabState> {
    ...
    if (!state) state = this.registerPage(page);
  ...
  async ensureAtLeastOne(): Promise<void> {
    if (this.tabs.size === 0) {
      await this.context.newPage();
    }
  }
