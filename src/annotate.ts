...
function buildSvgOverlay(
  ...
  for (const el of elements) {
    ...
    // Bounding box
    parts.push(
      `<rect x="${x}" y="${y}" width="${w}" height="${h}" fill="none" stroke="#FF3B30" stroke-width="2"/>`,
    );
    ...
    parts.push(
      `<rect x="${x}" y="${badgeY}" width="${badgeW}" height="${badgeH}" fill="#FF3B30"/>`,
    );
    parts.push(
      `<text x="${x + 4}" y="${badgeY + 13}" font-family="monospace" font-size="13" font-weight="bold" fill="#FFFFFF">${escapeXml(label)}</text>`,
    );
  }
