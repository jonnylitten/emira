...
| `upload_at_label N "path"` | Attach a file. Works with native OS file pickers, not just `<input type=file>`. Upload one at a time when order matters. Gated (see below): needs `MARKSMAN_ALLOW_ESCALATED=1` and `MARKSMAN_UPLOAD_ROOT` |
...
`run_javascript`, `upload_at_label`, `get_cookies`, `set_cookie`, and `clear_cookies` are disabled by default and refuse with a `PolicyError` that names the fix. Enable them with `MARKSMAN_ALLOW_ESCALATED=1`, or the "Allow escalated tools" toggle in plugin config. Turning the gate on is appropriate when you are deliberately driving a target you trust; leave it off for general browsing and scraping of pages you did not choose.

`upload_at_label` additionally requires `MARKSMAN_UPLOAD_ROOT` (plugin toggle: "Upload root directory") pointing at the directory that holds the files to upload. Paths outside that directory are refused.
...
- **OmniParser:** Only switch for canvas/WebGL UIs (Figma, Google Maps, games). Much slower: roughly 10-20s per inference on CPU, and the first call is slower still while the sidecar loads the model (timing details in README, Detectors). Enable with `detector="omniparser"` per call, or `MARKSMAN_DETECTOR=omniparser` for the whole session.
