...
claude plugin marketplace add /path/to/marksman
claude plugin install marksman@marksman-local
```

On first session a `SessionStart` hook installs Node deps (Playwright, sharp, MCP SDK) into `~/.claude/plugins/data/marksman-marksman-local/` and downloads the Chromium binary. ~30s once; subsequent sessions are instant.

To try it for a single session without installing:

```bash
claude --plugin-dir /path/to/marksman
```
...
```
/plugin configure marksman@marksman-local
```
...
| `headless` | `true` | Turn off to watch what marksman does in a visible window. |
...
```bash
bash /path/to/marksman/scripts/setup-omniparser.sh    # ~15–60 min, ~1GB of weights
```
...
```bash
# In the marksman repo: bump version in .claude-plugin/plugin.json, then:
claude plugin update marksman@marksman-local
```
...
| Symptom | Try |
|---|---|
| Tools don't appear in a new session | `claude plugin list` — confirm `marksman@marksman-local` is enabled. |
...
| Browser opens visibly when you want headless | `/plugin configure marksman@marksman-local` and toggle `headless` back to true. |
...
```bash
claude plugin uninstall marksman@marksman-local           # drops data dir by default
claude plugin marketplace remove marksman-local           # unregister the marketplace
```
