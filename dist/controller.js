...
export class Marksman {
    ...
    async runJavascript(code, awaitPromise = false) {
        const page = await getPage();
        ...
        console.error(`[marksman] run_javascript${awaitPromise ? " (await)" : ""}: ${code.slice(0, 200)}${code.length > 200 ? "…" : ""}`);
    ...
    async getPageText(label, mainOnly = false) {
        if (label !== undefined) {
