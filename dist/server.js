...
server.tool("get_cookies", "List cookies for the current session. Pass `urls` (one URL or an array) to filter to cookies the browser would send to those URLs. Omit to get every cookie in the context.", {
    urls: z
        .union([z.string().url(), z.array(z.string().url())])
        .optional()
        .describe("Filter to cookies matching these URLs. Omit to return all cookies."),
}, async ({ urls }) => {
    const cookies = await m.getCookies(urls);
    const summary = cookies
        .map((c) => `${c.name}=${c.value.slice(0, 40)}${c.value.length > 40 ? "…" : ""} (${c.domain}${c.path})`)
        .join("\n");
    return {
        content: [
            {
                type: "text",
                text: cookies.length === 0
                    ? "(no cookies)"
...
server.tool("set_cookie", "Set a cookie on the current browser context. Provide either `url` OR (`domain` AND optionally `path`). `expires` is a Unix timestamp in seconds; omit for a session cookie.", {
    name: z.string().min(1),
    value: z.string(),
...
server.tool("clear_cookies", "Clear cookies from the current browser context. Without filters, clears EVERYTHING. With filters (`name`, `domain`, `path`), narrows the scope. Use this for logout-like behavior without wiping the whole profile (which is what clear_profile does).", {
...
const shutdown = async () => {
    if (shuttingDown)
        return; // multiple triggers can race (SIGHUP + stdin close)
    shuttingDown = true;
    await closeBrowser();
    process.exit(0);
};
