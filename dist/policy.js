 ...
 */
export function escalationError(tool) {
    return (`${tool} is disabled. Set MARKSMAN_ALLOW_ESCALATED=1 (or the "Allow escalated tools" ` +
...
export function assertEscalationAllowed(tool) {
    if (!escalationEnabled())
        throw new PolicyError(escalationError(tool));
}
/**
 ...
 */
export function assertNavigable(raw) {
    let u;
    try {
        u = new URL(raw);
    }
    ...
    if (!["http:", "https:"].includes(u.protocol)) {
        throw new PolicyError(`scheme not allowed: ${u.protocol} (only http and https can be navigated; ` +
    ...
    if (allowed) {
        ...
        if (!list.includes(u.hostname.toLowerCase())) {
            throw new PolicyError(`host not in MARKSMAN_ALLOWED_HOSTS: ${u.hostname} (allowed: ${list.join(", ")})`);
        }
    }
    return u.toString();
}
 ...
 */
export function assertUploadPath(requested) {
    const root = process.env.MARKSMAN_UPLOAD_ROOT?.trim();
    if (!root) {
        throw new PolicyError(`upload_at_label is disabled. Set MARKSMAN_UPLOAD_ROOT (or the "Upload root directory" ` +
    ...
    if (realTarget !== realRoot && !realTarget.startsWith(realRoot + path.sep)) {
        throw new PolicyError(`upload path is outside MARKSMAN_UPLOAD_ROOT: ${requested} resolves to ` +
 ...
 */
export function fencePageContent(text, src) {
 ...
 */
export function ensureSecureDir(dir, label) {
    mkdirSync(dir, { recursive: true, mode: 0o700 });
    try {
        chmodSync(dir, 0o700);
    }
    catch (err) {
        throw new PolicyError(`refusing to use ${label} at ${dir}: could not restrict it to your user ` +
            `(${err.message}). This usually means another user owns that ` +
            `path. Set a different directory and restart.`);
    }
    const st = statSync(dir);
    if ((st.mode & 0o777) !== 0o700) {
        throw new PolicyError(`refusing to use ${label} at ${dir}: permissions are ` +
            `${(st.mode & 0o777).toString(8)} after chmod, expected 700.`);
    }
    if (typeof process.getuid === "function" && st.uid !== process.getuid()) {
        throw new PolicyError(`refusing to use ${label} at ${dir}: owned by uid ${st.uid}, not you ` +
